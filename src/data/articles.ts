export interface Article {
  slug: string;
  title: string;
  excerpt: string;
  cover: string;
  date: string;
  author: string;
  category: string;
  readTime: string;
  content: string;
}

export const ARTICLES: Article[] = [
  {
    slug: "tips-sewa-mobil-surabaya",
    title: "7 Tips Sewa Mobil di Surabaya Agar Perjalanan Aman & Nyaman",
    excerpt:
      "Menyewa mobil di Surabaya bisa jadi pengalaman yang menyenangkan jika Anda tahu cara memilih layanan yang tepat. Berikut tips lengkapnya.",
    cover: "/images/mobil-banyak.jpeg",
    date: "2026-04-08",
    author: "Admin Juragan",
    category: "Tips & Trik",
    readTime: "5 menit",
    content: `
## Mengapa Sewa Mobil di Surabaya?

Surabaya sebagai kota metropolitan terbesar kedua di Indonesia memiliki mobilitas tinggi. Baik untuk keperluan bisnis maupun wisata, menyewa mobil dengan sopir profesional adalah pilihan cerdas.

### 1. Pilih Penyedia Terpercaya

Pastikan Anda memilih rental mobil yang sudah memiliki reputasi baik. Cek ulasan pelanggan di Google atau media sosial. Juragan Rental Surabaya telah melayani ratusan pelanggan puas dengan rating bintang 5.

### 2. Periksa Kondisi Armada

Sebelum berangkat, pastikan kendaraan dalam kondisi prima. Di Juragan Rental, semua unit selalu dicek secara berkala dan dibersihkan sebelum digunakan.

### 3. Konfirmasi Rute dan Jadwal

Komunikasikan rute perjalanan Anda dengan jelas kepada driver. Sopir kami yang berpengalaman akan menyarankan rute tercepat dan terhindari dari kemacetan.

### 4. Pahami Sistem Pembayaran

Pastikan Anda memahami struktur biaya termasuk BBM, tol, dan parkir. Di Juragan Rental, kami menerapkan sistem harga transparan tanpa biaya tersembunyi.

### 5. Siapkan Dokumen Perjalanan

Untuk perjalanan luar kota, pastikan membawa identitas diri dan dokumen penting lainnya. Driver kami akan membantu mengurus kebutuhan perjalanan Anda.

### 6. Manfaatkan Layanan 24/7

Jangan ragu menghubungi admin kapan saja. Tim Juragan Rental siap membantu Anda selama 24 jam penuh melalui WhatsApp.

### 7. Booking Jauh Hari

Untuk mendapatkan unit terbaik, disarankan melakukan pemesanan minimal 1-2 hari sebelum keberangkatan, terutama pada musim liburan.

---

*Tertarik menyewa mobil di Surabaya? Hubungi Juragan Rental Surabaya sekarang juga dan nikmati perjalanan nyaman bersama sopir profesional!*
    `,
  },
  {
    slug: "destinasi-wisata-jawa-timur",
    title: "5 Destinasi Wisata Terbaik di Jawa Timur yang Wajib Dikunjungi",
    excerpt:
      "Jawa Timur menyimpan pesona alam yang luar biasa. Dari Gunung Bromo hingga Kawah Ijen, berikut destinasi yang bisa Anda jelajahi bersama Juragan Rental.",
    cover: "/images/pengantin.jpeg",
    date: "2026-04-05",
    author: "Admin Juragan",
    category: "Wisata",
    readTime: "7 menit",
    content: `
## Jelajahi Keindahan Jawa Timur

Jawa Timur adalah surganya wisata alam dan budaya di Indonesia. Dengan layanan sewa mobil dari Juragan Rental Surabaya, Anda bisa menjelajahi semua destinasi ini dengan nyaman.

### 1. Gunung Bromo

Sunrise di Gunung Bromo adalah pengalaman yang tak terlupakan. Hamparan lautan pasir dan kawah yang menakjubkan menjadikan Bromo destinasi wajib bagi wisatawan domestik maupun mancanegara.

**Tips dari kami**: Berangkat dari Surabaya pukul 00.00 untuk sampai di viewpoint sebelum matahari terbit.

### 2. Kawah Ijen

Fenomena blue fire yang hanya ada di dua tempat di dunia, salah satunya di Kawah Ijen, Banyuwangi. Pendakian malam hari akan memberikan pengalaman yang luar biasa.

### 3. Kota Batu & Jatim Park

Cocok untuk wisata keluarga! Kota Batu menawarkan berbagai wahana edukasi dan rekreasi seperti Jatim Park 1, 2, dan 3, Museum Angkut, serta Eco Green Park.

### 4. Pantai Papuma, Jember

Pantai eksotis dengan formasi batu karang yang menakjubkan. Jaraknya sekitar 4 jam dari Surabaya, sangat cocok untuk weekend trip bersama keluarga.

### 5. Air Terjun Madakaripura

Air terjun tertinggi di Pulau Jawa ini terletak di Probolinggo. Sering disebut sebagai "air terjun terindah" karena dikelilingi tebing-tebing yang meneteskan air dari segala arah.

---

*Siap menjelajahi Jawa Timur? Pesan paket wisata dari Juragan Rental Surabaya dan nikmati perjalanan tanpa ribet!*
    `,
  },
  {
    slug: "keuntungan-sewa-mobil-dengan-sopir",
    title: "Keuntungan Sewa Mobil dengan Sopir vs Self-Drive di Surabaya",
    excerpt:
      "Bingung memilih antara sewa mobil dengan sopir atau menyetir sendiri? Simak perbandingan lengkapnya di artikel ini.",
    cover: "/images/with-driver2.jpeg",
    date: "2026-04-01",
    author: "Admin Juragan",
    category: "Informasi",
    readTime: "4 menit",
    content: `
## Sewa dengan Sopir atau Self-Drive?

Ketika merencanakan perjalanan di Surabaya, salah satu pertimbangan penting adalah apakah Anda akan menyetir sendiri atau menggunakan jasa sopir. Mari kita bahas kelebihan masing-masing.

### Keuntungan Sewa dengan Sopir

**1. Bebas Stres di Jalan**
Anda tidak perlu pusing dengan navigasi dan kemacetan jalur kota Surabaya yang padat. Sopir profesional kami sudah sangat hafal setiap rute.

**2. Lebih Aman**
Sopir kami terlatih untuk berkendara secara aman dan defensif. Anda bisa fokus pada pekerjaan atau menikmati pemandangan tanpa khawatir.

**3. Fleksibilitas Tinggi**
Perlu berhenti mendadak atau mengubah rute? Tidak masalah! Sopir kami siap menyesuaikan dengan kebutuhan Anda.

**4. Tidak Perlu Tahu Jalan**
Sangat cocok untuk Anda yang baru pertama kali ke Surabaya atau tidak familiar dengan jalanan kota.

### Kapan Self-Drive Lebih Cocok?

Self-drive lebih sesuai jika Anda:
- Sudah sangat familiar dengan jalanan Surabaya
- Membutuhkan privasi penuh
- Memiliki SIM dan pengalaman berkendara yang cukup

### Kesimpulan

Untuk mayoritas kebutuhan bisnis dan wisata, **sewa mobil dengan sopir** adalah pilihan terbaik. Anda bisa duduk santai sementara sopir profesional mengantar Anda ke tujuan dengan aman dan nyaman.

---

*Butuh mobil dengan sopir profesional di Surabaya? Juragan Rental Surabaya siap melayani Anda 24/7!*
    `,
  },
  {
    slug: "panduan-airport-transfer-surabaya",
    title: "Panduan Lengkap Airport Transfer Bandara Juanda Surabaya",
    excerpt:
      "Tak perlu repot mencari transportasi dari bandara. Ini panduan airport transfer yang nyaman dan terpercaya dari Juragan Rental.",
    cover: "/images/with-driver3.jpeg",
    date: "2026-03-28",
    author: "Admin Juragan",
    category: "Panduan",
    readTime: "4 menit",
    content: `
## Airport Transfer Bandara Juanda

Bandara Internasional Juanda adalah pintu gerbang utama menuju Surabaya dan Jawa Timur. Mendapatkan transportasi yang aman dan nyaman dari bandara sangat penting untuk memulai perjalanan Anda dengan baik.

### Mengapa Memilih Airport Transfer?

**Kepastian Harga**: Berbeda dengan taksi atau ojol yang harganya fluktuatif saat jam sibuk, layanan airport transfer kami menawarkan harga tetap yang sudah disepakati di awal.

**Driver Menunggu di Terminal**: Sopir kami akan standby di area kedatangan dengan papan nama Anda. Tidak perlu repot mencari transportasi di tengah keramaian bandara.

**Kendaraan Premium**: Anda akan dijemput dengan unit bersih dan terawat, bukan kendaraan sembarangan.

### Area Layanan

Kami melayani transfer dari Bandara Juanda ke:
- **Surabaya Pusat** — 30-45 menit
- **Surabaya Barat/Timur** — 40-60 menit
- **Sidoarjo** — 15-25 menit
- **Gresik** — 45-60 menit
- **Malang** — 2-3 jam

### Cara Booking

1. Hubungi admin kami via WhatsApp
2. Informasikan nomor penerbangan dan jam kedatangan
3. Tentukan tujuan drop-off
4. Konfirmasi pembayaran
5. Driver akan standby 15 menit sebelum jadwal kedatangan

---

*Booking airport transfer sekarang dan mulai perjalanan Anda di Surabaya dengan nyaman!*
    `,
  },
];
